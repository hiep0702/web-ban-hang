...
class OrderController extends Controller
{
    ...
    {
        // Lấy ra thông tin hóa đơn
        DB::table('orderdetail')->where('OrderID', $orderid)->delete();
        DB::table('order')->where('OrderID', $orderid)->delete();
        $order = Order::join('orderstatus', 'orderstatus.OrderStatusID', '=', 'order.OrderStatusID')
            ->join('user', 'user.UserID','=','order.CustomerID')
            ->orderby('OrderDate','DESC')
            ->get();
        return view('admin.orders.info_order', compact('order'));
    }
